...
ssize_t process_http(SOCKET S, char *host, char *page, char *poststr)
{
    char sendline[MAXLINE + 1], recvline[MAXLINE + 1];
    ssize_t n;
    snprintf(sendline, MAXSUB,
...
mySock.sin_addr.S_un.S_addr =inet_addr("127.0.0.1");
S=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
int error=connect(S,(struct sockaddr *)&mySock,sizeof(mySock));
if(error==0)cout<<"connected"<<endl;
char *hname = "test";
char *page = "/p.php";
char *poststr = "login=test\r\n";